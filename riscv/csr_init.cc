...
void state_t::csr_init(processor_t* const proc, reg_t max_isa)
{
  ...
  auto dscratch0 = std::dynamic_pointer_cast<csr_t>(csrmap.find(CSR_DSCRATCH0)->second);
  auto dscratch1 = std::dynamic_pointer_cast<csr_t>(csrmap.find(CSR_DSCRATCH1)->second);
  add_ext_csr(EXT_SDSEC, CSR_SDSCRATCH0, std::make_shared<sdscratch_csr_t>(proc, CSR_SDSCRATCH0, dscratch0));
  add_ext_csr(EXT_SDSEC, CSR_SDSCRATCH1, std::make_shared<sdscratch_csr_t>(proc, CSR_SDSCRATCH1, dscratch1));
  ...
  add_csr(CSR_MDTCFG, mdtcfg = std::make_shared<mdtcfg_csr_t>(proc, CSR_MDTCFG));
